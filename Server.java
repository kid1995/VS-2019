...
public class Server {
			...
			do {
				try {
					...
					try {
						...
						clientInfos = clientInfos.stream().filter(
								clientInfo -> ((System.currentTimeMillis() - clientInfo.getLastActiveTime()) > 1000))
								.collect(Collectors.toList());

						System.out.println(clientInfos.size() + " are active");
						listClient = clientInfos.stream().map(ClientInfo::toString).collect(Collectors.toList());
						listClient.forEach(System.out::println);
	...
	public static void main(String args[]) {
		...
		/*
		 * ClientInfo c1 = new ClientInfo("1",2,1000); ClientInfo c2 = new
		 * ClientInfo("1",2,2000); ClientInfo c3 = new ClientInfo("1",2,3000);
		 * ClientInfo c4 = new ClientInfo("1",2,4000);
		 * 
		 * clientInfos.add(c1); clientInfos.add(c2); clientInfos.add(c3);
		 * clientInfos.add(c4); clientInfos = clientInfos.stream().filter(clientInfo ->
		 * clientInfo.getLastActiveTime()>3000).collect(Collectors.toList());
		 * 
